...
def getUniformPoints(num):
    i = 1
    all_fo = open("../dataset/uniform_" + str(num) + "_.csv", "w")
    for i in range(num):
        x = random.uniform(0, 1)
        y = random.uniform(0, 1)
        node_string = str(x) + "," + str(y) + "," + str(i) + "\n"
        i += 1
        all_fo.write(node_string)
    all_fo.close()

def getNormalPoints(num):
    longitudes = tf.truncated_normal([num,1], mean=0.5, stddev=0.25, dtype=tf.float32)
    latitudes = tf.truncated_normal([num,1], mean=0.5, stddev=0.25, dtype=tf.float32)
    with tf.Session() as sees:
        lati = sees.run(latitudes)
        longi = sees.run(longitudes)
        with open("../dataset/normal_" + str(num) + "_.csv", "w") as fo:
            for i in range(num):
                x = longi[i]
                y = lati[i]
                node_string = str(x[0]) + "," + str(y[0]) + "," + str(i) + "\n"
...
def getSkewedPoints(num, a):
    longitudes = tf.truncated_normal([num,1], mean=0.5, stddev=0.25, dtype=tf.float32)
    latitudes = tf.truncated_normal([num,1], mean=0.5, stddev=0.25, dtype=tf.float32)
    with tf.Session() as sees:
        lati = sees.run(latitudes)
        longi = sees.run(longitudes)
        with open("../dataset/skewed_" + str(num) + "_skewness_" + str(a) + "_.csv", "w") as fo:
            for i in range(num):
                x = longi[i]
                y = lati[i]
                node_string = str(x[0]) + "," + str(y[0]**a) + "," + str(i) + "\n"
                fo.write(node_string)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        size = int(sys.argv[1])
    getUniformPoints(size)
    getNormalPoints(size)
    # getSkewedPoints(size, 1)
    # getSkewedPoints(size, 3)
    # getSkewedPoints(size, 5)
    # getSkewedPoints(size, 7)
    # getSkewedPoints(size, 9)
